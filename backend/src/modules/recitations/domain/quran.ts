...
/**
 * The 114 surahs with their Hafs ayah counts. Single source of truth for the
 * searchable dropdown, the heart map, and ayah-range validation (spec 005).
 */
...
export const getSurah = (n: number): Surah | undefined => BY_NUMBER.get(n);

/** Validates a surah number and an ayah range against the real ayah count. */
export function assertValidRange(
  ...
  if (
    ...
    throw new BusinessRuleError(
      `Ayah range must satisfy 1 ≤ from ≤ to ≤ ${surah.ayahCount} for ${surah.name}`,
    );
  }
  return surah;
}
